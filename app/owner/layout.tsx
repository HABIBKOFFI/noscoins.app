...
const NAV = [
  ...
  { href: "/owner/venues", label: "Mes espaces" },
  { href: "/owner/quotes", label: "Devis" },
  { href: "/owner/bookings", label: "Réservations" },
  { href: "/owner/messages", label: "Messages" },
  { href: "/owner/documents", label: "Documents KYC" },
  { href: "/owner/payouts", label: "Virements" },
      ...
      <header className="bg-white border-b border-[#ede7e0]">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 h-14 flex items-center justify-between">
          <Link href="/" className="flex items-center gap-2 no-underline">
            ...
            <span className="text-sm font-bold text-[#1A1410]" style={{ fontFamily: "Playfair Display, serif" }}>
              Noscoins <span className="text-[#C4622D] font-normal text-xs ml-1">Propriétaire</span>
            </span>
          </Link>
          <form action="/api/auth/logout" method="post">
            <button type="submit" className="text-sm text-[#1A1410]/50 hover:text-[#C4622D]">
              Déconnexion
            </button>
          </form>
        </div>
